...
function inicio() {
  ...
  document.getElementById("logObjetos").style.display = "none";
}
...
function usuarios() {
  document.getElementById("urlSistema").innerText = "/ Inicio / Usuario";
  document.getElementById("opcoes").style.display = "none";
  document.getElementById("resumo").style.display = "none";
  document.getElementById("usuariosAdmin").style.display = "flex";

  const tabela = document
    .getElementById("tabelaUser")
    .getElementsByTagName("tbody")[0];
  tabela.innerHTML = "";
...
function excluirUsuario(id) {
  const response = fetch(`http://localhost:3333/users/remove/${id}`, {
    method: "DELETE",
  });
